...
<style>
    ...
    .card-call { border-left: 8px solid #238636 !important; }
    .card-put { border-left: 8px solid #da3633 !important; }
    
    .badge-extreme { background: rgba(46, 160, 67, 0.25); color: #3fb950; border: 1px solid #238636; padding: 5px 12px; border-radius: 20px; font-size: 11px; font-weight: 900; }
    .badge-high { background: rgba(56, 139, 253, 0.25); color: #58a6ff; border: 1px solid #1f6feb; padding: 5px 12px; border-radius: 20px; font-size: 11px; font-weight: 800; }
    .badge-medium { background: rgba(210, 153, 34, 0.25); color: #d29922; border: 1px solid #9e6a03; padding: 5px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; }
</style>
...
<div class="terminal-header">
    ...
    <p style="margin: 4px 0 0 0; color: #8b949e; font-size: 12px;">Single Asset Deep Scan & Precise Timestamp Engine</p>
</div>
""", unsafe_allow_html=True)
...
# Calculate Exact Start and End Timestamps based on current block
...
card_style = "card-call" if "CALL" in action else "card-put"
badge_style = "badge-extreme" if "98%" in accuracy else ("badge-high" if "92%" in accuracy else "badge-medium")

st.markdown(f"""
<div class="signal-card-solo {card_style}">
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
        <span class="{badge_style}">{accuracy}</span>
        <span style="font-size:12px; color:#3fb950; font-weight:700;">🟢 ACTIVE SIGNAL</span>
    </div>
    ...
    <div style="display:flex; justify-content:space-between; align-items:center;">
        ...
        <div style="text-align:right;">
            <h1 style="margin:0; font-size:24px; font-weight:900;">{action}</h1>
        </div>
    </div>
    ...
    <div style="display:flex; justify-content:space-between; background:rgba(0,0,0,0.3); padding:10px 15px; border-radius:10px; margin-bottom:12px;">
        <div>
            <span style="font-size:11px; color:#8b949e; display:block;">START TIME</span>
