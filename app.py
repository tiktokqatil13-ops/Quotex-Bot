...
<div class="terminal-header">
    ...
    <p style="margin: 4px 0 0 0; color: #8b949e; font-size: 12px;">Browser-Synced Local Time Engine</p>
</div>
""", unsafe_allow_html=True)
...
# MASTER BROWSER-SYNCED ENGINE (Handles Timer, Timestamps, and Dynamic Signals perfectly)
components.html(f"""
<div style="background: linear-gradient(145deg, #161b22 0%, #0d1117 100%); border-radius: 16px; border: 1px solid #30363d; padding: 20px; max-width: 700px; margin: 0 auto; box-shadow: 0 8px 30px rgba(0,0,0,0.5);">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
            ...
            <p style="margin: 4px 0 0 0; color: #8b949e; font-size: 13px;">Broker Sync: <code style="color: #58a6ff; font-size: 15px;">LIVE LOCAL</code></p>
        </div>
    ...
    <p id="reason-text" style="font-size: 12px; color: #8b949e; margin: 0;"><i>Initializing strategy engine...</i></p>
</div>
...
function updateEngine() {{
    ...
    // Dynamic Strategy Rotation per Candle Block
    if (blockStart !== lastBlock) {{
        lastBlock = blockStart;
        
        // Pseudo-random based on block timestamp
        let seed = blockStart;
        let actions = ["CALL (BUY) 🟢", "PUT (SELL) 🔴"];
        let actionColors = ["#3fb950", "#f85149"];
        let actionIdx = seed % 2;
        
        let strategies = [
            "Bollinger Band Extreme Rejection & RSI Oversold Confluence",
            "Upper Bollinger Band Resistance & RSI Overbought Divergence",
            "Fast EMA Ribbon (3/7/14) Momentum Expansion",
            "Support Level Hammer Pinbar Reversal Pattern",
            "Institutional Order Block Liquidity Sweep"
        ];
        let stratIdx = Math.floor(seed / 7) % strategies.length;
        
        document.getElementById("action-text").innerText = actions[actionIdx];
        document.getElementById("action-text").style.color = actionColors[actionIdx];
        document.getElementById("reason-text").innerHTML = "<i>Strategy #" + (seed % 9000 + 1000) + ": " + strategies[stratIdx] + "</i>";
        
        // Card border color update
        let container = timerEl.parentElement.parentElement.parentElement;
        container.style.borderLeft = "8px solid " + (actionIdx === 0 ? "#238636" : "#da3633");
    }}
}}
