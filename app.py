...
<div class="terminal-header">
    <div>
        ...
        <p style="margin: 3px 0 0 0; color: #8b949e; font-size: 13px;">Multi-Timeframe Smart Money Concepts (SMC) & Pattern Engine</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
def fetch_candles(symbol, timeframe="1m"):
    endpoints = [
        f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval={timeframe}&limit=500",
        f"https://api1.binance.com/api/v3/klines?symbol={symbol}&interval={timeframe}&limit=500",
        f"https://api2.binance.com/api/v3/klines?symbol={symbol}&interval={timeframe}&limit=500"
    ]
...
def analyze_market_deep(pair_label, timeframe="1m"):
    ...
    try:
        ...
        if ema20 > ema50 and (is_hammer or lower_sweep or is_bullish_engulfing) and c_close > ema20:
            ...
            return current_price, "CALL (BUY) 🟢", f"Trend + {pattern}", "95% SAFE", "HIGH"
            ...
            return current_price, "PUT (SELL) 🔴", f"Trend + {pattern}", "95% SAFE", "HIGH"

        elif is_doji:
            if c_close < ema200:
                return current_price, "CALL (BUY) 🟢", "Reversal: Doji Support", "80% RISKY", "MEDIUM"
            else:
                return current_price, "PUT (SELL) 🔴", "Reversal: Doji Resistance", "80% RISKY", "MEDIUM"

        elif is_shooting_star:
            return current_price, "PUT (SELL) 🔴", "Bearish Shooting Star", "80% RISKY", "MEDIUM"

        elif is_hammer:
            return current_price, "CALL (BUY) 🟢", "Bullish Hammer Support", "80% RISKY", "MEDIUM"

        return current_price, "ANALYZING ⏳", f"Scanning {timeframe} Structure...", "WAITING", "LOW"

    except Exception:
        return 0.0, "ANALYZING ⏳", "Recalculating...", "WAITING", "LOW"
...
for idx, label in enumerate(selected_labels):
    price, action, reason, accuracy, risk = analyze_market_deep(label, selected_tf)
...
# Convert selector TF to TradingView format (1m -> 1, 3m -> 3, 5m -> 5)
